...
\begin{axiom}{Peano公理}
	...
	1) $X \neq \varnothing$; \qquad 2) 存在$x$使得$x \notin S(X)$; \qquad 3) $S$是一个单射; 
...
B2)的四条结论和$N_0$的定义足以说明$N_0=\mathbb{N}$. 也即, 通过定义$\mathbb{N}$是最小的归纳集和利用Peano公理定义$\mathbb{N}$是完全等价的.
...
\begin{definition}{自然数集上的乘法}
	...
	1) $n\times 0=0$;\qquad 2) $n\times S(m)=n\times m+n$.
\end{definition}
...
\begin{definition}{自然数的次幂}
	...
	1) 对于$n \neq 0$, 有$n^0=1$, $n^{S(m)}=n^m \times n$; \qquad 2) 对于$n = 0$, 若$m \neq 0$则$n^m=0$.
\end{definition}
...
\begin{axiom}{环}
	...
	1) $(R,+)$构成Abel群; \qquad 2) $\bigcdot$满足结合性质; \qquad 3) $\bigcdot$对$+$有分配性质. 
...
\begin{axiom}{域}
	...
	1) $(F,+,\bigcdot)$是一个环; \qquad 2) $\bigcdot$满足单位元、逆元、交换性质. 
...
实际上, 我们可以证明$|\R| = |\mathcal{P}(\N)|$, 参见5.4.4小节. 
